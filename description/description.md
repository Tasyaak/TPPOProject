...
Для числовых признаков использовался векторизатор `DictVectorizer` с методом стандартизации `StandardScaler`, а для текстовых признаков использовался `TfidfVectorizer` с методом стандартизации `MaxAbsScaler`. В качестве подхода к обучению использовалась кросс-валидация `StratifiedKFold` с разбиением на 5 частей, для подбора гиперпараметров моделей использовался `HalvingGridSearchCV`.
...
Для первой модели с помощью `HalvingGridSearchCV` были выбраны следующие параметры:
```json
    "alpha": 0.001,
...
```
    ...
    accuracy                           0.96    165291
   macro avg       0.93      0.92      0.92    165291
weighted avg       0.96      0.96      0.96    165291
```
...
```
           ...
           1       0.92      0.98      0.95      7097
           2       0.61      0.28      0.39       799
           3       0.99      0.99      0.99      1795
           4       0.84      0.97      0.90       908
           5       0.93      0.67      0.78       480
          ...
          14       0.98      1.00      0.99       468
          15       1.00      1.00      1.00       386
          16       0.98      0.88      0.93       222
          17       0.91      1.00      0.95       266
          18       1.00      1.00      1.00      1023
          19       1.00      1.00      1.00       470

    accuracy                           0.96     29170
   macro avg       0.94      0.92      0.92     29170
weighted avg       0.96      0.96      0.96     29170
```
...
По полученным результатам можно увидеть, что модель справляется хуже всего с классификацией 2-го, 5-го и 9-го классов, ошибочно классифицируя экземпляры в соседние классы. Это связано с тем, что рекомендации, соответствующие данным классам и их соседям, связаны с одними теми же кодами ошибок компиляции.
