...
Среди моделей машинного обучения рассматривалась модель стохастического градиентного спуска с логистической функцией потерь. Для числовых признаков использовался векторизатор `DictVectorizer` с методом стандартизации `StandardScaler`, а для текстовых признаков использовался `TfidfVectorizer` с методом стандартизации `MaxAbsScaler`. Использовалось 2 подхода к обучению: кросс-валидация `StratifiedKFold` с разбиением на 5 частей и разбиение на обучающую, валидационную и тестовую выборки в соотношении `70:15:15`, для подбора гиперпараметров моделей использовались `GridSearchCV` и `HalvingGridSearchCV`.
...
Для модели, для обучения которой делалось разбиение на обучающую, валидационную и тестовую выборки, с помощью `GridSearchCV` были выбраны следующие параметры:
```json
    "alpha": 0.001,
    ...
    "penalty": "l2"
```
А для модели с кросс-валидацией с помощью `HalvingGridSearchCV` были выбраны следующие параметры:
```json
    "alpha": 0.001,
    "class_weight": "balanced",
    "max_iter": 800,
