...
FROM error_codes e
CROSS JOIN (
    SELECT 'Проверьте, что имя файла/библиотеки и путь к нему корректны' AS recommendation,
...
--     UNION ALL      всего 88 примеров, блииинь
...
--     третья рекомендация с тернаником (op = :)
) AS t
...
CROSS JOIN (
    SELECT 'Указанный оператор был некорректно использован, замените его на корректный' AS recommendation,
            'FIX_OPERATOR' AS recommendation_code
--     UNION ALL
--     SELECT 'Приведите левый операнд к ожидаемому типу (снимите const, если по смыслу допустимо), добавьте скобки или измените сигнатуру оператора',
--             'FIX_LEFT_OPERAND'
) AS t
...
FROM error_codes e
CROSS JOIN (
    SELECT 'Приведите правый операнд к ожидаемому типу (снимите const, если по смыслу допустимо), исправьте опечатку или измените сигнатуру оператора' AS recommendation,
            'FIX_RIGHT_OPERAND' AS recommendation_code
--     UNION ALL
--     SELECT 'Указанный оператор был некорректно использован, замените его на корректный',
--             'FIX_OPERATOR'
) AS t
